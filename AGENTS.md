...
- **Picture quality (tuned 2026-09-13 evening, after the owner said the picture was soft and the colours poor).** In order of how much each mattered:
  ...
  - **Filters on “กล้อง USB”**: `ปรับสี` (color_filter_v2) and `เพิ่มความคม` (sharpness 0.08). The colour filter is **a compensation for room light, so it is tuned against measurements, not taste**: with the room dark it ran gamma +0.45 / contrast +0.25 / saturation +0.2 to drag `YAVG` from 57 to 90, which also dragged up the noise. After the hotel added light (2026-09-13, late evening) the same brightness needed far less help, and it now sits at **gamma +0.15, contrast +0.20, saturation +0.15, brightness +0.06** — `YAVG 84`, visibly cleaner.
  - Re-tuning it is a sweep, not a guess: set a gamma, wait ~14s for the new frames to reach the live edge, measure, repeat. The numbers from that sweep (same scene, lit): gamma 0.45 → YAVG 90, 0.25 → 86, 0.10 → 83, 0.00 → 81. A scene that is mostly dark wall moves very little, which is itself the tell that the *scene* needs light rather than the filter needing more gain.
