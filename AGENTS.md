...
- **Two camera rigs, switchable from the phone (added 2026-09-14 at the owner's request).** The Camo sources were never deleted when the hotel moved to the USB camera, so bringing the phone back as a second angle was a matter of switching scene items rather than rebuilding anything.
  - A *rig* is a camera **and its mic together** — `กล้อง USB` + `ไมค์กล้อง USB`, or `กล้อง Camo (มือถือ)` + `ไมค์มือถือ (Camo)`. Switching video alone would leave the wrong microphone live, which is the kind of thing nobody notices until it is on air.
  - Actions `usbcam` / `phonecam` on `/control/api/command`, a button on the phone page. `setRig()` flips `SetSceneItemEnabled` on all four items; `activeRig()` reads the answer back **out of OBS** rather than remembering it, so a scene touched by hand in the OBS window cannot desync it.
  - `activeRig()` falls back to parsing `ROYS_Event.json` when OBS is not connected, because its most important caller runs before OBS is even launched: **the Camo-kill on start is now conditional.** Killing Camo Studio is right when the USB camera is going on air and exactly wrong when Camo *is* the camera.
  - The mic that mute and the level meter act on follows the rig (`obs.micInput` is reassigned on every status poll), and `status.camera` reports whichever camera is switched on, so the "camera" row and the `0x0 = no picture` check keep working for the phone too.
  - **Switching mid-event takes about 4 seconds**, not 30: the agent's poll interval already drops to `FAST_POLL_S` while the channel is streaming or someone has the control page open.
  - **Verified 2026-09-14 without a phone attached:** `phonecam` moved `rig` to `phone` with the Camo source reporting `0x0` (correct — nothing is feeding it), `usbcam` came back to `1920x1080`, and the scene file agreed. What is *not* verified is anything requiring the phone itself.
  - **The phone source had no device selected at all (found 2026-09-14).** `video_device_id` and `last_video_device_id` were both `""` — the source was detached when the hotel moved to the USB camera (the `ROYS_Event.json.before-camo-detach` backup is from that day) and switching the rig to it produced a `0x0` source forever. Two things were needed, in this order:
    1. **Get the device id from OBS, never type it.** `GetInputPropertiesListPropertyItems` with `propertyName: "video_device_id"` returns every camera with the exact string OBS wants. The stored form escapes every `#` as `#22` (`Camo:\\?\root#22camera#220000#22{guid}\{guid}`), which is precisely the escaping that has silently broken this scene before.
    2. **Restart OBS.** Setting the id through `SetInputSettings` on a running OBS updated the settings but never opened the device — no `[DShow Device: …] settings updated` line appeared in the log, and neither `deactivate_when_not_showing: false` nor toggling the scene item forced it. After a close and reopen the log showed `video device: Camo` and the source came up `1280x720`. Same family as the "do not change this camera's capture mode while OBS is running" trap: dshow device changes need a restart to take.
  - **A source reporting its full size still proves nothing about the picture.** With the device finally open, `1280x720 active=True`, the frame was solid black — `YAVG 16`, which is black in limited range, from both OBS and a direct `ffmpeg -f dshow -i video="Camo"` capture. Camo Studio was running and could see the phone (`device: Atsawin's iPhone`) but was not connected to it, and an unconnected Camo emits a black frame rather than nothing at all. Check brightness, not just dimensions — `status.camera.active` and the `0x0` test both pass on a black feed.
  - **Untested and the thing most likely to bite: whether Camo Studio and the USB camera can be open at once.** The documented failure is that a running Camo Studio holds the USB camera and OBS then fails with `Insufficient system resources`. That was with Camo configured onto a local camera; configured onto the phone it may leave the USB camera alone, but nobody has checked. Test it before an event, not during one. DroidCam is installed as well (`DroidCam Video` / `DroidCam Audio` both enumerate) and supports a USB cable, which is the obvious answer to the Wi-Fi drops that killed the phone rig the first time.
...
- **Startup latency: where the ~25 seconds between "start" and a picture actually goes.** Timed off the agent's own log on 2026-09-14, from a real start the owner triggered:
  ...
  - **Verified live on 2026-09-14:** the served playlist carries `#EXT-X-START:TIME-OFFSET=-6,PRECISE=YES` and steady-state segments are still 6s (`6.0, 6.0, 6.0, 7.166, 6.0, 6.034`), so the request rate is untouched. `hls_init_time` has not been confirmed yet — by the time the playlist was fetched, `seg_000000` had already rolled out of the window. Catch it by polling the playlist within the first few seconds of a start.
  - **After all that, the owner reports the remaining wait is on the TV, not the server.** Which narrows what is left to try, and rules one thing out:
    - **`hls_list_size` is not a lever.** The HLS rule is that a client starts no less than *three target durations* from the live edge — that is measured in seconds, not in list entries. Trimming the list to 3 leaves the player starting 18s back exactly as before, while removing the catch-up room a stalled TV needs. Do not "optimise" it.
    - **`hls_time` is the only real lever**, because it is what scales that three-target-duration rule. It divides the wait and multiplies the request count by the same factor:

      | `hls_time` | player waits | req/hour/viewer | viewer-hours/day | 20 TVs get |
      |---|---|---|---|---|
      | 6s (current) | ~18s | 1,200 | ~70 | 3.5 h |
      | 4s | ~12s | 1,800 | ~47 | 2.3 h |
      | 3s | ~9s | 2,400 | ~35 | 1.75 h |
