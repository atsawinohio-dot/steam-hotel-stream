...
1. **Light the room the camera points at.** This is the blocker for picture quality, and it is not a software problem — see "Picture quality" below for the measurements. As of 2026-09-13 21:00 the camera is still aimed at an unlit desk corner: the raw device reads `YAVG 8.7/255`, OBS's own render of the scene reads `YAVG 25.6`, and the tuner cannot lift it at all without the picture breaking up into colour noise.
2. **Verify channel 21 audio with real speech.** Run `workers/event-live/test-mic.ps1` (or `Test Mic.bat`) while someone talks near the camera — it needs no broadcast and no quota. A room with nobody speaking says nothing about speech levels, so the script now refuses to grade a clip with no speech in it.
3. The owner still needs to disable the laptop's built-in webcam in Device Manager (it wedges every camera when opened).
4. **The mic fader contradicts the gain filter** — `ไมค์กล้อง USB` carries a `+10 dB` gain filter but its mixer fader sits at **0.213 (−13.4 dB)**, so the chain nets **−3.4 dB**, not +10. Left alone deliberately: which way to correct it is a measurement, not a guess, and item 2 is that measurement.
...
- **Picture quality (tuned 2026-09-13 evening, after the owner said the picture was soft and the colours poor).** In order of how much each mattered:
  ...
  - **Filters on “กล้อง USB”**: `ปรับสี` (filter id `color_filter`, OBS's "Color Correction") and `เพิ่มความคม` (sharpness). **They are tuned to the room and the framing, against measurements, not by eye** — re-tune them whenever the camera is moved or the lighting changes.
    ...
    - Current in the scene file (2026-09-13): **gamma −0.07, contrast +0.16, saturation +0.22, brightness +0.06, sharpness 0.12**. These were tuned against the lit display wall (`YAVG 155, YHIGH 208`) and **do not apply to the present framing** — the camera is back on an unlit corner and the same values now render `YAVG 77` with the black floor at 74. Saturation is up because the frame is mostly white wall: `SATAVG` barely moves (5.2 → 5.7) while the figurines on the shelves visibly gain colour, so judge saturation by eye and use `SATAVG` only to check it is not runaway.
