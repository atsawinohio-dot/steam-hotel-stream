...
- **Picture quality (tuned 2026-09-13 evening, after the owner said the picture was soft and the colours poor).** In order of how much each mattered:
  ...
  - **Filters on “กล้อง USB”**: `ปรับสี` (color_filter_v2: gamma +0.45, contrast +0.25, saturation +0.2, brightness +0.06) and `เพิ่มความคม` (sharpness 0.08). These are a **compensation for a dark room, not a fix**: measured off the live channel with `ffmpeg -vf signalstats`, the picture went from `YAVG 57` (everything squeezed between 50 and 75 — badly underexposed) to `YAVG 91`, at the cost of visible noise. If the hotel lights the area properly, dial the gamma back down.
